...
/**
 * 默认选择转换器配置类 Created by Ambitor on 2019/1/21
 *
 * @param <S> 状态
 * @param <E> 事件
 */
public class DefaultChoiceTransitionConfigurer<S, E> extends BaseTransitionConfigurer<S, E>
    ...
    /**
     * action执行成功，状态扭转 source -> target 如抛出异常则状态不会扭转
     *
    ...
    /**
     * action执行成功状态从 source -> target 注意，如果有error函数，当action执行失败时会调用error，且如果error执行成功无异常后 状态也会扭转 source -> target
     */
    @Override
